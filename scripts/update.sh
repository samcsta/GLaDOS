...
#
# Set GLADOS_GIT=/path/to/git to override the Git binary. On macOS, the script
# prefers /usr/bin/git because Homebrew Git can break on fresh Intel Macs when
# its libcurl linkage is out of sync.
#
...
select_git() {
  if [[ -n "${GLADOS_GIT:-}" ]]; then
    GIT_BIN="$GLADOS_GIT"
  elif [[ "$(uname -s)" == "Darwin" && -x /usr/bin/git ]]; then
    GIT_BIN="/usr/bin/git"
  else
    GIT_BIN="$(command -v git || true)"
  fi

  [[ -n "$GIT_BIN" ]] || die "git not found in PATH"
  "$GIT_BIN" --version >/dev/null 2>&1 || die "git binary is not usable: $GIT_BIN"
}
...
preflight() {
  ...
  select_git
  note "Using git: $GIT_BIN ($("$GIT_BIN" --version))"
  ...
  local cur_branch
  cur_branch="$(git_cmd -C "$REPO_ROOT" rev-parse --abbrev-ref HEAD)"
  ...
  if ! git_cmd -C "$REPO_ROOT" diff --quiet || ! git_cmd -C "$REPO_ROOT" diff --cached --quiet; then
    if [[ "$FORCE" == "1" ]]; then
...
show_incoming() {
  ...
  git_cmd -C "$REPO_ROOT" fetch "$REMOTE" "$BRANCH"
  local range="HEAD..$REMOTE/$BRANCH"
  local count
  count="$(git_cmd -C "$REPO_ROOT" rev-list --count "$range" 2>/dev/null || echo 0)"
  if [[ "$count" == "0" ]]; then
    echo "Already up to date with $REMOTE/$BRANCH (0 incoming commits)."
  else
    echo "$count incoming commit(s):"
    git_cmd -C "$REPO_ROOT" log --oneline "$range"
    echo
    echo "Files that would change:"
    git_cmd -C "$REPO_ROOT" diff --stat "$range"
  fi
}

git_pull() {
  note "git pull $REMOTE $BRANCH"
  if ! git_cmd -C "$REPO_ROOT" pull --ff-only "$REMOTE" "$BRANCH"; then
