...
test('VPN-only mode serves updates without per-user application credentials', async () => {
  ...
  try {
    ...
    assert.match(landing.body.toString(), /mix-blend-mode:screen/);
    ...
    assert.match(landing.body.toString(), /https:\/\/git\.r3dt34m\.net\/scosta44\/glados\/src\/branch\/codex\/release-v4\.5\.8/);
    assert.doesNotMatch(landing.body.toString(), /github\.com\/samcsta\/GLaDOS/);
    assert.match(landing.body.toString(), /Open Windows v4\.5\.8 source/);
    assert.match(landing.body.toString(), /no official Windows binaries are published/i);
