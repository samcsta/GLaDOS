...
function renderDownloadPage(installerRoot, installerBasePath = '/installers') {
  ...
  const platforms = [
    ...
    },
    {
      ...
      sourceUrl: 'https://git.r3dt34m.net/scosta44/glados/src/branch/codex/release-v4.5.8',
    },
  ];
  const cards = platforms.map(platform => {
    ...
    if (platform.sourceUrl) {
      ...
      return `<article class="card">${heading}<div class="card-actions"><a class="download" href="${source}" rel="noreferrer">Open Windows v4.5.8 source</a><span class="hint">QA-approved branch · no official Windows binaries are published.</span></div></article>`;
    }
