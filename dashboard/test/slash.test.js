...
test('security-review context modes default to blind and are mutually exclusive', () => {
  ...
  assert.deepEqual(slash.parseSecurityReviewArg('/tmp/repo', fsStub), {
    ok: true, mode: 'blind', maxDurationMinutes: null, singleModel: null, target: '/tmp/repo', isLocalPath: true, isUrlOrDomain: false,
  });
  ...
  assert.deepEqual(slash.parseSecurityReviewArg('--blind "/tmp/repo with spaces"', fsStub), {
    ok: true, mode: 'blind', maxDurationMinutes: null, singleModel: null, target: '/tmp/repo with spaces', isLocalPath: true, isUrlOrDomain: false,
  });
