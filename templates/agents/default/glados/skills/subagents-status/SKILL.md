...
The built-in `/subagents` command returned empty results during engagements — it read `~/.agent-sdk/subagents/runs.json`, which lagged real session state and missed agents that exited dirty. The operator now runs a local GLaDOS Ops Dashboard at `http://localhost:4280` which tails every agent's live JSONL and applies a liveness check (session status=running AND JSONL mtime within last 2 min AND no endedAt).
